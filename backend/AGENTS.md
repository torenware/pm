...
- `app/ai.py` owns the KodeKloud OpenAI-compatible client, strict board-operation response schema, and controlled provider errors.
...
- Validate AI operations before applying them, enforce board ownership, and apply each response atomically.
- Accept conversation history from the browser for the current request only; never persist it.
